 ...
 */
 class KVStore extends StorageElement{
     ...
     * Cost: 1x read
     */
    async retrieve(chain: any[], getOptions: KVNamespaceGetOptions<any>): Promise<unknown>{
        if(chain.length === 0)
            return undefined
     ...
     */
    async assign(chain: any[], value: any, putOptions: KVNamespacePutOptions, getOptions: KVNamespaceGetOptions<any>): Promise<boolean>{
        if(chain.length === 0)
            return false
        if(chain.length === 1)
     ...
     *  Sub-item: 1x read + 1x write
     */
    async remove(chain: any[], putOptions: KVNamespacePutOptions, getOptions: KVNamespaceGetOptions<any>): Promise<boolean | void>{
        if(chain.length === 0)
            return undefined
        if(chain.length === 1)
     ...
     */
    async keys(chain: any[], listOptions: KVNamespaceListOptions, getOptions: KVNamespaceGetOptions<any>): Promise<string[]>{
        if(chain.length === 0)
            return undefined
     ...
     *      KV: 1x read
     */
    async entries(chain: any[], listOptions: KVNamespaceListOptions, getOptions: KVNamespaceGetOptions<any>): Promise<any[][]>{
        if(chain.length === 0)
        return undefined
